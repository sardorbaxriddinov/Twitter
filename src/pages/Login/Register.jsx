...
const Register = () => {
  ...
  function handleRegisterSubmit(e){
    ...
    setUsers([...users, data])
    setTimeout(() => {toast.success("Successfully created")}, 500);
    setTimeout(() => {
      e.target.reset()
      setIsLoading(false)
      navigate(-1)
  ...
  return (
    <div className='w-[670px] mx-auto mt-[60px] '>
      ...
      <form onSubmit={handleRegisterSubmit} className='flex flex-col space-y-[25px] mb-10' autoComplete='off'>
        ...
        <Input defaultValue={"+998"} name={"phone"} type={"tel"} placeholder={"Phone number"} />
        <Input name={"password"} type={"password"} placeholder={"Password"} />
        {useEmail && <Input name={"email"} type={"email"} placeholder={"Email"} />}
        <div className='flex items-center justify-between my-[30px]'>
          <p onClick={() => setUseEmail(!useEmail)} className='text-[#1DA1F2] cursor-pointer text-[18px]'>{useEmail ? "Hide email" : "Use email"}</p>
